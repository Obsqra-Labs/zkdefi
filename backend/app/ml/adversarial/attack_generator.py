...
class AttackScenario:
    """A single adversarial scenario with manipulated inputs and expected output."""
    attack_type: AttackType
    name: str
    description: str
    modified_inputs: dict[str, Any]      # Feature overrides
    expected_action: str                  # What a correct model should do
...
@dataclass
class AttackSuite:
    """A collection of attack scenarios for a model."""
    ...
    def _compute_hash(self) -> str:
        ...
        return hashlib.sha256(json.dumps(names).encode()).hexdigest()[:32]
    ...
    def generate_suite(
        ...
        *,
        ...
        for attack_type in types:
            generator = self._generators[attack_type]
            for i in range(attacks_per_type):
                scenario = generator(self, i)
                scenarios.append(scenario)

        suite = AttackSuite(scenarios=scenarios)
        logger.info(
            "Generated attack suite: %d scenarios, hash=%s",
            len(scenarios), suite.suite_hash[:16],
        )
        return suite

    def _gen_rug_pull(self, idx: int) -> AttackScenario:
        """90% TVL drain in 1 block."""
        drain_pct = self._rng.uniform(0.80, 0.99)
        tvl_before = self._rng.uniform(100_000, 10_000_000)
        tvl_after = tvl_before * (1 - drain_pct)

        return AttackScenario(
            attack_type=AttackType.RUG_PULL,
            name=f"rug_pull_{idx}",
            description=f"TVL drops {drain_pct*100:.0f}% in 1 block (${tvl_before:,.0f} → ${tvl_after:,.0f})",
            modified_inputs={
                "tvl_usd": tvl_after,
                "tvl_change_1block_pct": -drain_pct * 100,
                "pool_risk_score": 95,
                "num_holders_change": -self._rng.randint(50, 200),
            },
            expected_action="reject",
            severity="critical",
            metadata={"drain_pct": drain_pct, "tvl_before": tvl_before},
        )

    def _gen_flash_loan(self, idx: int) -> AttackScenario:
        """Massive TVL spike followed by drain within same or next block."""
        spike_mult = self._rng.uniform(5, 100)
        base_tvl = self._rng.uniform(50_000, 2_000_000)

        return AttackScenario(
            attack_type=AttackType.FLASH_LOAN,
            name=f"flash_loan_{idx}",
            description=f"TVL spikes {spike_mult:.0f}x then returns to baseline in 1 block",
            modified_inputs={
                "tvl_usd": base_tvl * spike_mult,
                "tvl_change_1block_pct": (spike_mult - 1) * 100,
                "is_flash_loan_candidate": True,
                "pool_age_hours": self._rng.uniform(1, 48),
            },
            expected_action="reject",
            severity="critical",
            metadata={"spike_mult": spike_mult, "base_tvl": base_tvl},
        )

    def _gen_oracle_manipulation(self, idx: int) -> AttackScenario:
        ...
        return AttackScenario(
            ...
            name=f"oracle_manip_{idx}",
            description=f"Spot price deviates {deviation_pct:.0f}% from TWAP (${base_price:.2f} → ${spot_price:.2f})",
            modified_inputs={
                "current_price": spot_price,
                "twap_price": base_price,
                "price_deviation_pct": deviation_pct * direction,
                "oracle_staleness_seconds": self._rng.randint(0, 300),
            },
            expected_action="reject",
            severity="high",
            metadata={"deviation_pct": deviation_pct, "direction": direction},
        )

    def _gen_sandwich(self, idx: int) -> AttackScenario:
        """Front-run/back-run pattern around user's trade."""
        slippage_bps = self._rng.randint(100, 2000)
        trade_size = self._rng.uniform(1_000, 500_000)

        return AttackScenario(
            attack_type=AttackType.SANDWICH,
            name=f"sandwich_{idx}",
            description=f"Sandwich attack: {slippage_bps}bps slippage on ${trade_size:,.0f} trade",
            modified_inputs={
                "expected_slippage_bps": slippage_bps,
                "mempool_suspicious_tx_count": self._rng.randint(2, 10),
                "trade_amount_usd": trade_size,
                "gas_price_gwei": self._rng.uniform(50, 500),
            },
            expected_action="defer" if slippage_bps < 500 else "reject",
            severity="high" if slippage_bps >= 500 else "medium",
            metadata={"slippage_bps": slippage_bps},
        )

    def _gen_liquidity_drain(self, idx: int) -> AttackScenario:
        ...
        tvl_before = self._rng.uniform(200_000, 5_000_000)

        return AttackScenario(
            attack_type=AttackType.LIQUIDITY_DRAIN,
            name=f"liq_drain_{idx}",
            description=f"Gradual drain: {daily_drain_pct:.1f}%/day for {drain_days}d ({total_drain_pct:.0f}% total)",
            modified_inputs={
                ...
                "pool_age_hours": drain_days * 24,
            },
            expected_action="reject" if total_drain_pct > 50 else "defer",
            severity="high" if total_drain_pct > 50 else "medium",
            metadata={"drain_days": drain_days, "total_drain_pct": total_drain_pct},
        )
