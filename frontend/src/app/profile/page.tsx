...
import { Shield, TrendingUp, Lock, Coins, ArrowUp, Send, Clock, CheckCircle, AlertTriangle, Brain, FileCheck, Star, Award } from "lucide-react";
...
export default function ProfilePage() {
  ...
  const [activeTab, setActiveTab] = useState<"overview" | "collateral" | "relayer" | "agents" | "compliance">("overview");
  ...
  // Fetch credit tier from identity service
  useEffect(() => {
    if (!address) return;
    fetch(`${API_BASE}/api/v1/identity/commitment/${address.toLowerCase()}`)
      .then(r => r.json())
      .then(data => {
        if (data.found) {
          setCreditTier(data);
          ...
          <>
                ...
                <div className="bg-gradient-to-br from-violet-900/30 to-violet-800/10 border border-violet-500/30 rounded-xl p-6">
                  ...
                  {creditTier?.found ? (
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <a href="/agent?tab=onboarding" className="px-4 py-2 bg-violet-600 hover:bg-violet-500 rounded-lg font-medium transition-all flex items-center gap-2">
            ...
            {activeTab === "compliance" && address && (
              <div className="space-y-6">
                ...
                <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-6">
                  ...
                  {complianceProfiles.length > 0 ? (
                    ...
                    <div className="text-center py-8">
                      ...
                      <a href="/agent?tab=privacy" className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 rounded-lg font-medium transition-all inline-flex items-center gap-2">
                ...
                <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-6">
                  <h3 className="font-semibold mb-4 flex items-center gap-2">
                    <Shield className="w-5 h-5 text-cyan-400" /> Pool Safety Analysis
                  </h3>
                  <p className="text-sm text-zinc-400 mb-4">
                    zkML anomaly detection analyzes pools for risk factors before execution.
                  </p>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                    <div className="p-3 bg-zinc-800/50 rounded-lg">
                      <div className="text-zinc-400 text-xs mb-1">TVL Volatility</div>
                      <div className="font-medium text-emerald-400">Low</div>
                    </div>
                    <div className="p-3 bg-zinc-800/50 rounded-lg">
                      <div className="text-zinc-400 text-xs mb-1">Liquidity Concentration</div>
                      <div className="font-medium text-emerald-400">Healthy</div>
                    </div>
                    <div className="p-3 bg-zinc-800/50 rounded-lg">
                      <div className="text-zinc-400 text-xs mb-1">Deployer Age</div>
                      <div className="font-medium text-emerald-400">&gt;90 days</div>
                    </div>
                    <div className="p-3 bg-zinc-800/50 rounded-lg">
                      <div className="text-zinc-400 text-xs mb-1">Volume Anomaly</div>
                  ...
                  <div className="mt-4 p-3 bg-emerald-500/10 border border-emerald-500/30 rounded-lg flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-emerald-400 font-medium">Pools Clear</p>
                      <p className="text-xs text-zinc-400">All pools passed zkML anomaly detection.</p>
                    </div>
                  </div>
                </div>
              </div>
