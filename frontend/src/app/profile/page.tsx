          ...
          <>
            ...
            {activeTab === "reputation" && (
              <div className="space-y-6">
                ...
                {/* On-Chain Reputation (live contract reads) */}
                <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-6">
                  <h2 className="text-lg font-semibold mb-4 flex items-center gap-2">
                    <Shield className="w-5 h-5 text-blue-400" /> On-Chain Reputation
                    {onChainRep?.source === "on-chain" && (
                      <span className="text-xs bg-emerald-900/60 text-emerald-300 px-2 py-0.5 rounded-full ml-2">Live</span>
                    )}
                  </h2>
                  {onChainRepLoading ? (
                    <div className="text-sm text-zinc-500">Loading on-chain data…</div>
                  ) : onChainRep ? (
                    <div className="space-y-4">
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        <div className="bg-zinc-800/50 rounded-lg p-3">
                          <div className="text-xs text-zinc-500 mb-1">Reputation Score</div>
                          <div className="text-xl font-bold text-emerald-400">{onChainRep.reputation_score}</div>
                        </div>
                        <div className="bg-zinc-800/50 rounded-lg p-3">
                          <div className="text-xs text-zinc-500 mb-1">On-Chain Tier</div>
                          <div className={`text-xl font-bold ${onChainRep.tier === 0 ? "text-blue-400" : onChainRep.tier === 1 ? "text-emerald-400" : "text-orange-400"}`}>{onChainRep.tier_name}</div>
                        </div>
                        <div className="bg-zinc-800/50 rounded-lg p-3">
                        ...
                        <div className="bg-zinc-800/50 rounded-lg p-3">
                          ...
                          <div className="text-xl font-bold text-cyan-400">{onChainRep.collaborative_score}</div>
                        </div>
                      </div>
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                        <div className="bg-zinc-800/50 rounded-lg p-3">
                          <div className="text-xs text-zinc-500 mb-1">Successful / Failed TXs</div>
                          <div className="text-sm font-mono">
                            <span className="text-emerald-400">{onChainRep.successful_txns}</span>
                            <span className="text-zinc-600"> / </span>
                            <span className="text-red-400">{onChainRep.failed_txns}</span>
                          </div>
                        </div>
                        <div className="bg-zinc-800/50 rounded-lg p-3">
                        ...
                        <div className="bg-zinc-800/50 rounded-lg p-3">
                          ...
                          <div className="flex items-center gap-2">
                            {onChainRep.can_use_relayer ? (
                              <span className="text-emerald-400 text-sm font-medium flex items-center gap-1"><CheckCircle className="w-3.5 h-3.5" /> Enabled</span>
                            ) : (
                              <span className="text-zinc-500 text-sm">Disabled</span>
                            )}
                            {onChainRep.relayer_delay_seconds > 0 && (
                              <span className="text-xs text-zinc-500">({onChainRep.relayer_delay_seconds}s delay)</span>
                            )}
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="text-sm text-zinc-500">On-chain reputation data unavailable. Connect your wallet to view live contract data.</div>
                  )}
                </div>
                ...
                <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-6">
                  ...
                  {canUpgrade ? (
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-emerald-400 font-medium">Eligible for upgrade</p>
                        <p className="text-sm text-zinc-400">{userRep?.successful_txns} successful transactions</p>
                      </div>
