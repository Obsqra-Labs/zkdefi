...
import { SessionWallet, type SessionKeyInfo } from "./SessionWallet";
...
import { CapitalOSHome, type L3Identity } from "./CapitalOSHome";
...
type DemoPhase = "explore" | "onboarding" | "strategize" | "executing" | "live";
...
const PHASE_STEPS: { key: DemoPhase; label: string; icon: typeof Brain }[] = [
  { key: "explore", label: "Explore", icon: Brain },
  { key: "strategize", label: "Identity", icon: Fingerprint },
  { key: "executing", label: "Execute", icon: Zap },
  { key: "live", label: "Live", icon: Layers },
];
...
export function CapitalOSSection() {
  ...
  /* phase machine */
  const [phase, setPhase] = useState<DemoPhase>("explore");
  const [error, setError] = useState<string | null>(null);
  const [guestMode, setGuestMode] = useState(false);
  const [transitioning, setTransitioning] = useState(false);
  const [sidebarTab, setSidebarTab] = useState<"brain" | "identity">("brain");
  ...
  const address = isGuest ? DEMO_ADDRESS : walletAddress;
  const hasIdentity = !!identity;
  const panelAddress = identity?.wallet_address ?? DEMO_ADDRESS;
  const panelSession = identity?.session_id ?? DEMO_SESSION_ID;
  const panelL3 = identity?.l3_address ?? DEMO_L3_ADDRESS;

  /* ═══ actions ═══ */

  const smoothTransition = useCallback((to: DemoPhase) => {
    setTransitioning(true);
    setTimeout(() => { setPhase(to); setTransitioning(false); }, 300);
  }, []);
  ...
  const handleExecute = useCallback(async () => {
    ...
    try {
      const body: Record<string, unknown> = { wallet_address: address, risk_profile: selectedProfile };
  ...
  const handleStrategySelected = useCallback(
    (profile: string, _strategy: { portfolio_value_usd: number }) => { setSelectedProfile(profile); },
    [],
  );

  const handlePnLUpdate = useCallback(
    (data: { value: number; pnl: number; pnlPct: number; positionCount: number }) => {
    ...
    <div className="space-y-6">
      ...
      <div className="mx-auto max-w-2xl text-center">
        ...
        <h2 className="text-2xl font-bold leading-tight text-zinc-100 sm:text-3xl">
          Your DeFi Brain.<br className="hidden sm:block" /> Reputation. Strategy. Proofs.
        </h2>
      ...
      {isGuest && (
        <div className="mx-auto flex max-w-lg items-center justify-between rounded-lg border border-amber-500/20 bg-amber-500/5 px-4 py-2">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setIsModalOpen(true)} className="text-[10px] text-amber-400 underline hover:text-amber-200">
              Connect wallet
            </button>
            <button onClick={reset} className="text-[10px] text-zinc-500 hover:text-zinc-300">Exit</button>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[260px_1fr]">
        ...
        {/* ── Left sidebar: tabbed Brain | Identity ── */}
        <div className="lg:sticky lg:top-4 lg:self-start space-y-3">
          <div className="flex rounded-lg border border-zinc-800 bg-zinc-900/50 overflow-hidden">
            <button
              onClick={() => setSidebarTab("brain")}
              className={`flex-1 flex items-center justify-center gap-1.5 px-3 py-2 text-[10px] font-semibold uppercase tracking-wider transition-colors ${
                sidebarTab === "brain" ? "bg-zinc-800 text-zinc-200" : "text-zinc-600 hover:text-zinc-400"
              }`}
            >
            ...
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-1.5 px-3 py-2 text-[10px] font-semibold uppercase tracking-wider transition-colors ${
                sidebarTab === "identity" ? "bg-zinc-800 text-zinc-200" : "text-zinc-600 hover:text-zinc-400"
              }`}
            >
              <Fingerprint className="h-3 w-3" /> Identity
              {hasIdentity && <span className="h-1.5 w-1.5 rounded-full bg-emerald-400" />}
            </button>
          </div>
              ...
              {hasIdentity ? (
                <>
                  ...
                  {reputation && (
                    <details className="group">
                      <summary className="flex cursor-pointer items-center gap-2 rounded-lg border border-zinc-800 bg-zinc-900/50 px-3 py-2 text-[10px] font-semibold uppercase tracking-wider text-zinc-400 hover:text-zinc-200">
                ...
                <div className="rounded-xl border border-zinc-800 bg-zinc-900/30 p-4 space-y-3">
                  <div className="text-center">
                    <Fingerprint className="mx-auto h-8 w-8 text-zinc-600 mb-2" />
                    <p className="text-xs font-semibold text-zinc-300">No Identity Yet</p>
                    <p className="mt-1 text-[10px] text-zinc-500">
                      Connect your wallet or try as guest to get an L3 identity, reputation scan, and session key.
                    </p>
                  </div>
                  <div className="flex flex-col gap-2">
                    {isConnected && walletAddress ? (
                      <button
                        ...
                        className="flex items-center justify-center gap-2 rounded-lg border border-emerald-500/30 bg-emerald-600/10 px-4 py-2.5 text-xs font-semibold text-emerald-400 transition-all hover:border-emerald-400 hover:bg-emerald-600/20"
                      >
                        <Fingerprint className="h-3.5 w-3.5" /> Onboard
                      </button>
                    ) : (
                      <>
                        <button
                        ...
                        <button
                          ...
                          className="flex items-center justify-center gap-2 rounded-lg border border-zinc-700 bg-zinc-900/50 px-4 py-2 text-[11px] text-zinc-400 transition-all hover:border-zinc-500 hover:text-zinc-200"
                        >
                          <Eye className="h-3.5 w-3.5" /> Try as Guest
                        </button>
                      </>
                    )}
                  </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-0.5 text-xs text-zinc-500">
                {phase === "explore" && "Aggregated on-chain data · AI-scored · Verifiable execution"}
                {phase === "onboarding" && "Scanning reputation & provisioning identity…"}
                {phase === "strategize" && "Compare strategies · Select capital · Execute on paper"}
                {phase === "executing" && "Opening positions & settling to L3…"}
                {phase === "live" && "Live paper trading · P&L tracking · Settlement proofs"}
              </p>
            </div>
            {!hasIdentity && phase === "explore" && (
            ...
            {phase === "live" && (
              <button onClick={reset} className="text-xs text-zinc-500 transition-colors hover:text-zinc-300">
                ← Start over
              </button>
            )}
          </div>
          ...
          <div className={`transition-all duration-300 ${transitioning ? "opacity-0 translate-y-2" : "opacity-100 translate-y-0"}`}>
              ...
              <div className="space-y-5">
                ...
                <div className="rounded-xl border border-emerald-500/15 bg-emerald-950/10 p-5">
                  <div className="flex flex-col items-center gap-3 text-center">
                    <p className="text-sm font-semibold text-zinc-200">Ready to take control?</p>
                    <p className="text-xs text-zinc-500 max-w-md">
                      Connect your wallet to get a reputation scan, L3 identity, and session key —
                      then compare strategies and execute on paper with real pool data.
                    </p>
                    <div className="flex flex-wrap items-center justify-center gap-3">
                      {isConnected && walletAddress ? (
                        <button
                          onClick={handleOnboard}
                          className="group inline-flex items-center gap-3 rounded-xl border border-emerald-500/30 bg-emerald-600/10 px-6 py-3 text-sm font-semibold text-emerald-400 transition-all hover:border-emerald-400 hover:bg-emerald-600/20 hover:shadow-lg hover:shadow-emerald-500/10"
                        >
                        ...
                        <>
                          ...
                          <button
                            onClick={handleGuestOnboard}
                            className="group inline-flex items-center gap-2 rounded-xl border border-zinc-700 bg-zinc-900/50 px-5 py-3 text-sm text-zinc-400 transition-all hover:border-zinc-500 hover:text-zinc-200"
                          >
                            <Eye className="h-4 w-4" /> Try as Guest
                          </button>
                        </>
                      )}
                    </div>
            ...
            {phase === "onboarding" && (
              <div className="flex flex-col items-center gap-3 py-16">
                ...
                <p className="text-sm text-zinc-400">
                  Scanning reputation &amp; provisioning L3 identity
                  {walletAddress && (
                    <> for <span className="font-mono text-zinc-300">{walletAddress.slice(0, 6)}…{walletAddress.slice(-4)}</span></>
                  )}
                </p>
            ...
            {/* ═══ STRATEGIZE ═══ */}
            {phase === "strategize" && identity && (
              <div className="space-y-5">
                <div className="space-y-2">
                  <p className="text-xs font-semibold text-zinc-400">
                    Select hypothetical capital
                    <span className="ml-2 text-[10px] font-normal text-zinc-600">(or skip to use real portfolio)</span>
                  </p>
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {hypotheticalUsd && (
                      <button onClick={() => setHypotheticalUsd(null)} className="rounded-lg border border-zinc-700 px-2 py-1.5 text-[10px] text-zinc-500 hover:text-zinc-300">
                        Clear
                      </button>
                    )}
                  </div>
            ...
            {phase === "executing" && (
              <div className="flex flex-col items-center gap-3 py-16">
                ...
                <p className="text-sm text-zinc-400">Opening paper positions &amp; settling snapshot to L3…</p>
                <div className="flex flex-wrap justify-center gap-2 text-[10px] text-zinc-600">
                ...
                >
                  {isGuest ? (
                    <div className="space-y-3 p-4">
                      ...
                      <p className="border-t border-zinc-800 pt-2 text-center text-[10px] text-zinc-600">
                        Guest mode — connect wallet for live P&amp;L tracking
                      </p>
                    </div>
                  ) : (
                    <PnLTracker sessionId={execution.session_id} onUpdate={handlePnLUpdate} pollInterval={30_000} />
                  )}
                </TerminalPanel>
                ...
                >
                  <div className="space-y-4 p-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-bold uppercase tracking-wider text-emerald-400">Paper Trade Executed</span>
                      <span className="ml-auto text-[10px] text-zinc-500">{execution.positions_opened} positions · {selectedProfile}</span>
                    </div>
                    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                      <div className="rounded-lg border border-zinc-800 bg-zinc-900/50 px-4 py-2">
                        ...
                        <p className="mt-0.5 break-all font-mono text-[10px] text-zinc-400">{execution.snapshot?.snapshot_hash ?? "—"}</p>
                      </div>
      ...
      {/* ══════════════════════════════════════════════════════════════ */}
      {/* ─── Always-visible 2×2 Terminal Panels ───────────────────── */}
      {/* ══════════════════════════════════════════════════════════════ */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        >
          <DarkVaultPanel walletAddress={panelAddress} sessionId={panelSession} />
        </TerminalPanel>
        ...
        >
          <ProofOfPerformance walletAddress={panelAddress} sessionId={panelSession} />
        </TerminalPanel>
        ...
        >
          <IntelligentStream walletAddress={panelAddress} pollIntervalMs={10000} />
        </TerminalPanel>
      </div>
