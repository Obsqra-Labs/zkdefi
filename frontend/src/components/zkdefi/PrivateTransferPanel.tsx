...
export function PrivateTransferPanel({ onCommitmentsChange }: PrivateTransferPanelProps = {}) {
  ...
  const [commitmentFelt, setCommitmentFelt] = useState<string | null>(null);
  ...
  const handleGeneratePrivateDeposit = async () => {
    ...
    try {
      ...
      if (data.commitment) {
        ...
        setCommitmentFelt(data.commitment_felt ?? data.commitment);
  ...
  const handleSignAndSubmitDeposit = async () => {
    ...
    setStep(4);
    try {
      const commitmentForContract = commitmentFelt ?? commitment;
      const commitmentFeltBigInt = BigInt(commitmentForContract);
