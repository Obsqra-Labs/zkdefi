...
export function ShieldedPoolPanel({ onCommitmentsChange }: { onCommitmentsChange?: () => void }) {
  ...
  const handleGenerateDepositProof = async () => {
    ...
    try {
      ...
      if (data.commitment) {
        ...
        setDepositNonce(data.nonce ?? null);
        setStep(3);
  ...
  const handleSignDeposit = async () => {
    ...
    try {
      ...
      console.log("=== SHIELDED POOL DEPOSIT ===");
      console.log("Commitment u256:", { low: commitmentLow.toString(), high: commitmentHigh.toString() });
      
      // Human-signed: Only privacy proof needed, no execution proof
      // New u256 contract signature: private_deposit_u256(commitment_low, commitment_high, amount_low, amount_high, proof_len, ...proof)
      const result = await account.execute([
        ...
        },
        {
          ...
          entrypoint: "private_deposit_u256",
          calldata: [
  ...
  const handleGenerateWithdrawProof = async () => {
    ...
    const selectedObj = commitments.find((c) => c.commitment === selectedCommitment);
    try {
      const amountWei = (BigInt(amount) * BigInt(1e18)).toString();
      ...
      const res = await fetch(`${API_BASE}/api/v1/zkdefi/shielded_withdraw`, {
        ...
        body: JSON.stringify({
          ...
          ...(selectedObj?.balance != null && { balance: selectedObj.balance }),
          ...(selectedObj?.nonce != null && { nonce: selectedObj.nonce }),
        }),
      });
  ...
  const handleSignWithdraw = async () => {
    ...
    try {
      ...
      // Parse nullifier and commitment as u256 (low, high) for BN254 Poseidon compatibility
      const nullifierBigInt = BigInt(nullifier);
      const nullifierLow = nullifierBigInt % BigInt(2 ** 128);
      const nullifierHigh = nullifierBigInt / BigInt(2 ** 128);
      ...
      console.log("=== SHIELDED POOL WITHDRAW ===");
      console.log("Nullifier u256:", { low: nullifierLow.toString(), high: nullifierHigh.toString() });
      console.log("Commitment u256:", { low: commitmentLow.toString(), high: commitmentHigh.toString() });

      let result: { transaction_hash: string };
      if (useRelayer) {
        // Request relayed withdrawal (executed later by relayer) - not yet u256 compatible
        result = await account.execute({
          contractAddress: SHIELDED_POOL_ADDRESS as `0x${string}`,
          entrypoint: "request_relayed_withdraw",
          calldata: [
            nullifierLow.toString(),
            nullifierHigh.toString(),
            commitmentLow.toString(),
            commitmentHigh.toString(),
            amountLow.toString(),
            amountHigh.toString(),
        ...
        // Direct withdrawal with u256 interface
        result = await account.execute({
          contractAddress: SHIELDED_POOL_ADDRESS as `0x${string}`,
          entrypoint: "private_withdraw_u256",
          calldata: [
            nullifierLow.toString(),
            nullifierHigh.toString(),
            commitmentLow.toString(),
