  ...
  useEffect(() => {
    ...
    const fetchPositions = async () => {
      try {
        ...
        // Add private commitments from ALL localStorage sources
        let privateTotal = 0;
        let privateCount = 0;
          ...
          try {
            ...
            if (stored) {
              ...
              for (const c of commitments) {
                const amt = parseFloat(c.amount || c.balance || "0");
                privateTotal += amt;
                privateCount++;
              }
            }
        ...
        // Add private position to results if any
        if (privateTotal > 0) {
          results.push({
            protocol: "private",
            value: privateTotal,
            color: "#f59e0b", // amber for private
          });
        }
        ...
        // Update aggregated position state for privacy view
        if (privateCount > 0) {
          setAggregatedPosition({
            total_value: validPositions.reduce((sum, p) => sum + p.value, 0).toString(),
            public_positions_count: validPositions.filter(p => p.protocol !== "private").length,
            private_commitments_count: privateCount,
          });
        }
      } catch (e) {
