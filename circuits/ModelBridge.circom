 ...
 * ModelBridge Circuit
 *
 * Bridges EZKL ML model proofs into the Groth16/Garaga verification pipeline.
 ...
 * ModelBridgeVerifier
 *
 * Main circuit for bridging EZKL ML proofs.
 * Uses 8 model outputs by default.
 *
 * Outputs (example):
 *   0-7: ML model inference outputs (logits, probabilities, etc.)
 */
template ModelBridgeVerifier() {
    var N_OUTPUTS = 8;
...
component main {public [expected_model_hash, output_lower_bound, output_upper_bound, timestamp]} = ModelBridgeVerifier();
