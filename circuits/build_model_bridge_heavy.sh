#!/usr/bin/env bash
# Build ModelBridgeHeavy circuit: compile → PoT → Groth16 setup → export vkey
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BUILD_DIR="$SCRIPT_DIR/build"
CIRCUIT_NAME="ModelBridgeHeavy"
mkdir -p "$BUILD_DIR"
...
if [ ! -f "$POT" ]; then
  ...
  if [ -f "$BUILD_DIR/pot12.ptau" ]; then
    ...
    echo "$ENTROPY" | npx snarkjs powersoftau contribute \
      ...
      --name="obsqra-model-bridge-heavy" -v
...
echo "$ENTROPY2" | npx snarkjs zkey contribute \
  ...
  --name="obsqra-model-bridge-heavy-contribution" -v
...
echo ""
echo "=== ModelBridgeHeavy circuit built successfully ==="
