...
const WALLET =
  "0x10eb4fb373daf7fc8985aa3185d9b1aebb4b2025111895d12668e5ca0447d76";
...
function mockRpc(opts: {
  nonce?: number;
  classHash?: string | Error;
  blockNumber?: number;
  /** Called once per getEvents() invocation. Returns pages to yield. */
  getEventsImpl?: (filter: unknown, callIndex: number) => unknown[][];
}): StarknetRPC {
  let reqCount = 0;
  let eventsCallIndex = 0;

  return {
    getNonce: async (_addr: string) => {
      reqCount++;
      return opts.nonce ?? 0;
    },
    getClassHashAt: async (_addr: string) => {
      reqCount++;
      if (opts.classHash instanceof Error) throw opts.classHash;
...
describe("getLiquidationCount", () => {
  ...
  it("counts liquidation events where wallet is the liquidated user (data[1])", async () => {
    ...
    const rpc = mockRpc({
      blockNumber: 100_000,
      getEventsImpl: (_f, callIndex) => {
        if (callIndex === 0) return [[SUPPLY_EVENT]];      // supply call
        if (callIndex === 1) return [[LIQUIDATION_EVENT]]; // liquidation call
        return [[]];
      },
    });
